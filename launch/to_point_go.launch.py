...
def launch_setup(context, *args, **kwargs):
    ...
    module_arg = context.launch_configurations['module']
    if not module_arg in ["1", "2"]:
        sys.exit("No module number provided")

    place_arg = context.launch_configurations['place']
    if not place_arg in ["entrance", "center"]:
        sys.exit("No place provided")
    ...
    ld = LaunchDescription([
        Node(
            ...
            executable='to_point_go',
            name='to_point',
            parameters=[{'coordinates_file_path': coordinates_file_path, 'place': place_arg, 'module': module_arg}],
            output='screen',
        )
    ])
...
def generate_launch_description():
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            'module',
            description='You need to provide which module in the world to launch.'
        ),
        DeclareLaunchArgument(
            'place',
            description='You need to provide the place of the world to launch.'
        ),
